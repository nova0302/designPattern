...
//Client calls this public static member function wehn it needs an instance
...
imageType input[NUM_ENTRIES] = {LSAT, LSAT, LSAT, SPOT, LSAT, SPOT, SPOT, LSAT};

int main(int argc, char *argv[]) {

  Image *images[NUM_ENTRIES];

  // Given an image type, find the right prototype, and return a clone
  for (int i=0; i<NUM_ENTRIES; i++)
    images[i] = Image::findAndClone(input[i]);

  // Demonstrate that correct image object have been cloned
  for (int i=0; i<NUM_ENTRIES; i++)
    images[i]->draw();
