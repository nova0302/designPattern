...
private:
  enum States{
    Received, DenyAllKnowledge, ReferClientToFacilities, FacilitiesHasNotSentPaperowrk,
    ElectricianIsNotDone, ElectricianDidItWrong, DispatchTechnician,
    SignedOff, DoesNotWork, FixElectricansWiring,Complete
  };
  int _state;
};
...
private:
  enum States{
    Receivec, AssignToEngineer, EngineerResearches, RequestIsNotPossible,
...
int main(int argc, char *argv[]) {
  ...
  cout << "job completed after only" << facilities.getNumberOfCalls() << " phone calls"
       << endl;
  return 0;
}
