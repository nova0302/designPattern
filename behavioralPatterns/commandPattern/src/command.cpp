...
public:
  ...
  void talk(){
    ...
    cmd.execute(); // ask the "black box" to callback the reveiver
  }
  void passOn(){
...
int main(int argc, char *argv[]) {
  //Fred will "execute" Barney which will result ina call to passOn()
  //Barney will "execute" Betty which will result in a call to gossip()
  //Barney will "execute" Wilma which will result in a call to listen()
  Person wilma("Wilma", Command());
  //2. Instantiate an objet for each "callback"
